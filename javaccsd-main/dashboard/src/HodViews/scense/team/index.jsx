    ...
    const handleDelete = (userId) => {
        ...
        axios.delete(`${API_BASE_URL}/api/users/${userId}`, {
            ...
            .then((response) => {
                ...
                setTeamDetails((prevDetails) => prevDetails.filter(user => user.id !== userId));
            })
            .catch((error) => {
                // Handle error
            ...
            renderCell: ({row}) => {
                return (
                    <Box width="40%"
                         ...
                         m={0.5}
                         alignItems="center"
                         backgroundColor={row.access === "admin" ? colors.redAccent[600] : colors.redAccent[700]}
                         borderRadius="4px"
                    >
                        <Button
                            variant="contained"
        ...
        <Box>
            ...
            <Box>
                <DataGrid
                    ...
                    sx={{
                        '& .MuiDataGrid-cell': {
                            fontSize: '13px', // Change the font size for all cells
                        },
                        '& .MuiDataGrid-columnHeader': {
                            fontSize: '16px', // Change the font size for column headers
                        },
                    }}
                />
            </Box>
